...
Public BrowseFolder_FolderName As String
Public BrowseFolder_Successful As Boolean

Public Sub BrowseFolder(hWnd As Long, windowTitle As String)
    BrowseFolder_Successful = False
    ...
    If IDList <> 0 Then
       BrowseFolder_FolderName = Space(MAX_PATH)
       Call SHGetPathFromIDList(IDList, BrowseFolder_FolderName)
       BrowseFolder_FolderName = Left(BrowseFolder_FolderName, InStr(BrowseFolder_FolderName, vbNullChar) - 1)
       BrowseFolder_Successful = True
    End If
End Sub
